<template>
  <div>
    ...
    <v-layout row wrap>
      <v-flex d-flex md12>
        <v-card v-if="data_loaded" :color="card_color" class="restaurant-card-new">
          ...
          <div class="widget-content-new">
            ...
            <v-card-text class="d-block text-center pt-0">
              <span class="display-2">{{current_value}}</span><span
              class="headline"> {{current_feed.measurement.short}}</span>
            </v-card-text>
            <v-card-title primary class="d-block title text-center pt-0 pb-0">{{$t("message.network_average")}}:
            </v-card-title>
            <v-card-text class="d-block text-center pb-0">
